...
export default function ContactDrawer({ open, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setServerErr(data.error ?? "Unknown error");
        setStatus("error");
  ...
  return (
    <FocusTrap
      active={!!open}
      focusTrapOptions={{
        initialFocus: "#contact-email-input", // primo campo
                ...
                {status !== "success" && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <Checkbox
                        label={f("check0")}
                        checked={interests.includes(f("check0"))}
                        onChange={() => toggleInterest(f("check0"))}
                      />
                      <Checkbox
                        label={f("check2")}
                        checked={interests.includes(f("check2"))}
                        onChange={() => toggleInterest(f("check2"))}
                      />
                      <Checkbox
                        label={f("check3")}
